...
function danger(i){state.choice=i;const r=state.route,c=characters[r];let body=r==="hoshi"?["「……その言葉、魔法より嬉しいな」彼の指が、そっとあなたの手袋を直す。","「もちろん。けれど、失敗したらぼくが悲しみます」笑顔は少しも崩れない。","「ほかの人？　ああ……必要、ありますか？」彼の杖が小さく鳴った。"][i]:r==="citrus"?["「承知しました。では“常に一緒にいる”規則を、今から作りましょう」","「それは規則違反です。……いえ、今のは聞かなかったことにします」","「許可なく面会を？　困ります。あなたの安全が最優先ですから」"][i]:["「うれしいです。でも依存ではありませんよ、保護です」ネムリは穏やかに微笑む。","「無理をしないで。あなたはもう、たくさん頑張りましたから」","「その人は、あなたを疲れさせませんか？」ネムリの声だけが、少し低くなる。"][i];scene({who:c.name,body,bg:c.visual,gain:42,opts:[{label:"……あなたの言う通りにする",go:()=>finalScene(0)},{label:"やっぱり帰らせて",go:()=>finalScene(1)},{label:"それでも、外へ行く",go:()=>finalScene(2)}]});}
function finalScene(i){state.choice=(state.choice+i)%3;const c=characters[state.route];scene({who:c.name,body:state.route==="hoshi"?"「大丈夫。怖がらないで。ここから先は、二人だけの最高のショーです」":state.route==="citrus"?"「ご安心ください。あなたが困らないよう、すべての規則を整えました」":"「眠ってしまえば、もう誰にも傷つけられませんよ」",bg:c.visual,gain:55,opts:[{label:"目を閉じる",go:ending}]});}
