...
const story = $("story-screen"), visual=$("visual"), panel=document.querySelector(".dialogue-panel"), speaker=$("speaker"), text=$("text"), choices=$("choices"), next=$("next-button"), meter=$("meter"), fill=$("meter-fill"), value=$("meter-value"), status=$("meter-status");
let continueScene = null;
const actionChoices = new Set(["紫のスポットライトへ","まぶしい白い庭園へ","ふわふわの屋根の保健室へ","この世界のことを聞く","少しだけ距離をとる","目を閉じる"]);

function show(which){ if(which!=="story") continueScene=null; Object.values(screens).forEach(s=>s.classList.add("hidden")); screens[which].classList.remove("hidden"); }
...
function scene({who="",body,opts=[],bg="daily",gain=0,onContinue=null}){ continueScene=onContinue; panel.classList.toggle("reply-only",!!onContinue); panel.classList.toggle("intro",!state.route); visual.className="visual "+bg+(state.level>=70?" danger":""); speaker.textContent=who; text.textContent=body; text.scrollTop=0; choices.innerHTML=""; choices.scrollTop=0; next.classList.toggle("hidden",!onContinue); next.onclick=onContinue; setLevel(gain); opts.forEach((o,i)=>{const b=document.createElement("button");b.className="choice";b.textContent=actionChoices.has(o.label)||o.label.startsWith("「")?o.label:`「${o.label}」`;b.onclick=()=>o.go(i);choices.appendChild(b)}); }
document.onclick=e=>{if(continueScene&&!e.target.closest("button")){const go=continueScene;continueScene=null;go();}};
function start(){state={route:null,level:0,choice:0,sound:state.sound};story.classList.remove("broken");meter.classList.add("hidden");show("story");scene({body:"――落ちる。\n見知らぬ空から降ってきたあなたの下に、三つの光が見えた。\nどこへ落ちる？",bg:"sky",opts:[{label:"紫のスポットライトへ",go:()=>pick("hoshi")},{label:"まぶしい白い庭園へ",go:()=>pick("citrus")},{label:"ふわふわの屋根の保健室へ",go:()=>pick("nemuri")} ]});}
const reactions = {
  hoshi: {
...
function reply(body,after,gain=0){const c=characters[state.route];scene({who:c.name,body,bg:c.visual,gain,onContinue:after});}
...
function daily(i){const r=state.route,c=characters[r];const body=r==="hoshi"?"ホシくんは紅茶を手品で花に変え、あなたの笑う顔をじっと見ていた。\n「不思議だよね。キミがいるだけで、この部屋は完璧なんだ」":r==="citrus"?"シトラスは庭を案内しながら、歩幅まであなたに合わせてくれた。\n「迷子にならないよう、私の右側を歩いてください」":"ネムリはあなたのために甘い薬草茶を淹れてくれた。\n「外は疲れますから、今日はここで休みましょうね」";reply(reaction("pick",i),()=>scene({who:c.name,body,bg:"daily",opts:[{label:"あなたとずっと一緒にいたい。大好き！",go:()=>bond(0)},{label:"この世界のことを聞く",go:()=>bond(1)},{label:"うるさいな。放っておいて、好きに歩かせろよ",go:()=>bond(2)}]}),18);}
function bond(i){const r=state.route,c=characters[r];const body=r==="hoshi"?"空中庭園を歩くと、ホシくんは街の人々から何度も声をかけられる。\nけれど彼は、誰にも返事をしなかった。あなたの手を引くことだけに夢中だった。":r==="citrus"?"昼食の席で、シトラスはナプキンの角度まで整えてくれる。\n「大丈夫です。貴女が困らないよう、私が全部覚えています」":"ネムリはあなたの髪を優しく梳かして、眠気の残る目元を覗き込む。\n「今日はよく眠れました？　外の夢は、見なくてもいいんですよ」";reply(reaction("daily",i),()=>scene({who:c.name,body,bg:"daily",opts:[{label:"もっと甘やかして。あなたがいちばん好き",go:()=>danger(0)},{label:"少しだけ距離をとる",go:()=>danger(1)},{label:"べたべた触るな。ほかの奴のところへ行くから",go:()=>danger(2)}]}),16);}
function danger(i){state.choice=i;const r=state.route,c=characters[r];const response=r==="hoshi"?["「……その言葉、魔法より嬉しいよ」","「もちろんだよ。けれど、失敗したらボクが悲しむだろう？」","「ほかの人？　ああ……必要、あるのかな？」"][i]:r==="citrus"?["「承知しました。では“貴女と常に一緒にいる”規則を、今から作りましょう」","「それは規則違反です。……いえ、今のは聞かなかったことにします」","「許可なく面会を？　困ります。貴女の安全が最優先ですから」"][i]:["「うれしいです。でも依存ではありませんよ、保護です」","「無理をしないで。あなたはもう、たくさん頑張りましたから」","「その人は、あなたを疲れさせませんか？」"][i];const body=r==="hoshi"?["彼の指が、そっとあなたの手袋を直す。","笑顔は少しも崩れない。","彼の杖が小さく鳴った。"][i]:r==="citrus"?"シトラスは笑顔のまま、新しい規則を書き足していく。":"ネムリは穏やかに微笑む。けれど保健室の扉は、いつの間にか閉じていた。";reply(response,()=>scene({who:c.name,body,bg:c.visual,opts:[{label:"あなたが好き。ずっとそばにいさせて",go:()=>finalScene(0)},{label:"やっぱり帰らせて",go:()=>finalScene(1)},{label:"ふざけるな！　お前の言いなりにはならない",go:()=>finalScene(2)}]}),42);}
function finalScene(i){state.choice=(state.choice+i)%3;const c=characters[state.route];const body=state.route==="hoshi"?"「大丈夫だよ。怖がらないで。ここから先は、二人だけの最高のショーなんだ」":state.route==="citrus"?"「ご安心ください。貴女が困らないよう、すべての規則を整えました」":"「眠ってしまえば、もう誰にも傷つけられませんよ」";reply(reaction("danger",i),()=>scene({who:c.name,body,bg:c.visual,opts:[{label:"目を閉じる",go:ending}]}),55);}
