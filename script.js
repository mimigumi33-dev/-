...
function pick(route){state.route=route;meter.classList.remove("hidden");const c=characters[route];scene({who:c.name,body:route==="hoshi"?"おや。空からお客さま？　……ふふ、歓迎するよ。ぼくはスターマイン。ホシくんと呼んでくれるだろう？":route==="citrus"?"お怪我はありませんか？　ここは雲上庭園です。ぼくはシトラス。まずは安全規則をご説明しますね。":"まあ。痛いところはありませんか？　ここは夢の保健室。ネムリと申します。どうぞ、こちらへ。",bg:c.visual,gain:12,opts:[{label:"助けてくれてありがとう",go:()=>daily()},{label:"ここがどこか教えてほしい",go:()=>daily()},{label:"帰る方法を探したい",go:()=>daily()}]});}
function daily(){const r=state.route,c=characters[r];let body=r==="hoshi"?"ホシくんは紅茶を手品で花に変え、あなたの笑う顔をじっと見ていた。\n「不思議だよね。きみがいるだけで、この部屋は完璧なんだ」":r==="citrus"?"シトラスは庭を案内しながら、歩幅まであなたに合わせてくれた。\n「迷子にならないよう、ぼくの右側を歩いてください」": "ネムリはあなたのために甘い薬草茶を淹れてくれた。\n「外は疲れますから、今日はここで休みましょうね」";scene({who:c.name,body,bg:"daily",gain:18,opts:[{label:"もっと一緒にいたい",go:()=>bond()},{label:"この世界のことを聞く",go:()=>bond()},{label:"少し散歩をしたい",go:()=>bond()}]});}
function bond(){const r=state.route,c=characters[r];const body=r==="hoshi"?"空中庭園を歩くと、ホシくんは街の人々から何度も声をかけられる。\nけれど彼は、誰にも返事をしなかった。あなたの手を引くことだけに夢中だった。":r==="citrus"?"昼食の席で、シトラスはナプキンの角度まで整えてくれる。\n「大丈夫です。あなたが困らないよう、ぼくが全部覚えています」": "ネムリはあなたの髪を優しく梳かして、眠気の残る目元を覗き込む。\n「今日はよく眠れました？　外の夢は、見なくてもいいんですよ」";scene({who:c.name,body,bg:"daily",gain:16,opts:[{label:"その優しさに甘える",go:()=>danger(0)},{label:"少しだけ距離をとる",go:()=>danger(1)},{label:"ほかの人の様子を尋ねる",go:()=>danger(2)}]});}
function danger(i){state.choice=i;const r=state.route,c=characters[r];let body=r==="hoshi"?["「……その言葉、魔法より嬉しいよ」彼の指が、そっとあなたの手袋を直す。","「もちろんだよ。けれど、失敗したらぼくが悲しむだろう？」笑顔は少しも崩れない。","「ほかの人？　ああ……必要、あるのかな？」彼の杖が小さく鳴った。"][i]:r==="citrus"?["「承知しました。では“常に一緒にいる”規則を、今から作りましょう」","「それは規則違反です。……いえ、今のは聞かなかったことにします」","「許可なく面会を？　困ります。あなたの安全が最優先ですから」"][i]:["「うれしいです。でも依存ではありませんよ、保護です」ネムリは穏やかに微笑む。","「無理をしないで。あなたはもう、たくさん頑張りましたから」","「その人は、あなたを疲れさせませんか？」ネムリの声だけが、少し低くなる。"][i];scene({who:c.name,body,bg:c.visual,gain:42,opts:[{label:"……あなたの言う通りにする",go:()=>finalScene(0)},{label:"やっぱり帰らせて",go:()=>finalScene(1)},{label:"それでも、外へ行く",go:()=>finalScene(2)}]});}
function finalScene(i){state.choice=(state.choice+i)%3;const c=characters[state.route];scene({who:c.name,body:state.route==="hoshi"?"「大丈夫だよ。怖がらないで。ここから先は、二人だけの最高のショーなんだ」":state.route==="citrus"?"「ご安心ください。あなたが困らないよう、すべての規則を整えました」":"「眠ってしまえば、もう誰にも傷つけられませんよ」",bg:c.visual,gain:55,opts:[{label:"目を閉じる",go:ending}]});}
