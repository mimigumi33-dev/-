...
function pick(route){state.route=route;meter.classList.remove("hidden");const c=characters[route];scene({who:c.name,body:route==="hoshi"?"おや。空からお客さま？　……ふふ、歓迎するよ。ボクはスターマイン。ホシくんと呼んでくれるだろう？":route==="citrus"?"お怪我はありませんか？　ここは雲上庭園です。私はシトラス。まずは安全規則をご説明しますね。":"まあ。あなた、痛いところはありませんか？　ここは夢の保健室。ぼくはネムリです。どうぞ、こちらへ。",bg:c.visual,gain:12,opts:[{label:"助けてくれてありがとう！　あなたに会えてうれしい",go:()=>daily()},{label:"ここがどこか教えてほしい",go:()=>daily()},{label:"勝手に近づくな。帰り道だけ教えろよ",go:()=>daily()}]});}
function daily(){const r=state.route,c=characters[r];let body=r==="hoshi"?"ホシくんは紅茶を手品で花に変え、あなたの笑う顔をじっと見ていた。\n「不思議だよね。キミがいるだけで、この部屋は完璧なんだ」":r==="citrus"?"シトラスは庭を案内しながら、歩幅まであなたに合わせてくれた。\n「迷子にならないよう、私の右側を歩いてください」": "ネムリはあなたのために甘い薬草茶を淹れてくれた。\n「外は疲れますから、今日はここで休みましょうね」";scene({who:c.name,body,bg:"daily",gain:18,opts:[{label:"あなたとずっと一緒にいたい。大好き！",go:()=>bond()},{label:"この世界のことを聞く",go:()=>bond()},{label:"うるさいな。放っておいて、好きに歩かせろよ",go:()=>bond()}]});}
function bond(){const r=state.route,c=characters[r];const body=r==="hoshi"?"空中庭園を歩くと、ホシくんは街の人々から何度も声をかけられる。\nけれど彼は、誰にも返事をしなかった。あなたの手を引くことだけに夢中だった。":r==="citrus"?"昼食の席で、シトラスはナプキンの角度まで整えてくれる。\n「大丈夫です。貴女が困らないよう、私が全部覚えています」": "ネムリはあなたの髪を優しく梳かして、眠気の残る目元を覗き込む。\n「今日はよく眠れました？　外の夢は、見なくてもいいんですよ」";scene({who:c.name,body,bg:"daily",gain:16,opts:[{label:"もっと甘やかして。あなたがいちばん好き",go:()=>danger(0)},{label:"少しだけ距離をとる",go:()=>danger(1)},{label:"べたべた触るな。ほかの奴のところへ行くから",go:()=>danger(2)}]});}
