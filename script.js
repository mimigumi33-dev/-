...
function pick(route){state.route=route;meter.classList.remove("hidden");const c=characters[route];scene({who:c.name,body:route==="hoshi"?"おや。空からお客さま？　……ふふ、歓迎するよ。ボクはスターマイン。ホシくんと呼んでくれるだろう？":route==="citrus"?"お怪我はありませんか？　ここは雲上庭園です。私はシトラス。まずは安全規則をご説明しますね。":"まあ。あなた、痛いところはありませんか？　ここは夢の保健室。ぼくはネムリです。どうぞ、こちらへ。",bg:c.visual,gain:12,opts:[{label:"助けてくれてありがとう",go:()=>daily()},{label:"ここがどこか教えてほしい",go:()=>daily()},{label:"帰る方法を探したい",go:()=>daily()}]});}
...
function finalScene(i){state.choice=(state.choice+i)%3;const c=characters[state.route];scene({who:c.name,body:state.route==="hoshi"?"「大丈夫だよ。怖がらないで。ここから先は、二人だけの最高のショーなんだ」":state.route==="citrus"?"「ご安心ください。貴女が困らないよう、すべての規則を整えました」":"「眠ってしまえば、もう誰にも傷つけられませんよ」",bg:c.visual,gain:55,opts:[{label:"目を閉じる",go:ending}]});}
